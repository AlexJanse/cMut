...
#'   "cMut") }
identifyAndAnnotateClusters <- function(dataTable,                               maxDistance,
...
addLinkPatterns <- function(table,                refHeader,
  ...
  table <- dplyr::mutate(table,
                         linkedPatterns = purrr::map2(tempMutColumn,
                                                      is.clustered,
                                               function(x, y) {
  ...
  table <- dplyr::mutate(table,
                         linkedPatterns = purrr::map(linkedPatterns,
                                                     function(x) {
                                                       x[[1]]
                                                     }))
  ...
  table <- dplyr::mutate(table,
                         is.linked = purrr::map_lgl(linkedPatterns,
                                                    function(x){
                                                      x[[1]][[1]] != "" &
                                                      x[[1]][[1]] != "NA"
                                                    }))
...
createIdentTable <- function(dataTable,      maxDistance,
  ...
  dataTable <- dplyr::mutate(dataTable,
                             is.clustered     = purrr::map2_lgl(is.clustered,
                                                                !!rlang::sym(refHeader),
                                                                function(x, y) {
                                                                  x & y != "N"
                                                                }),
...
#'   \code{\link{identifyAndAnnotateClusters}} function.
